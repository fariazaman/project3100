...
<html>
	...
	<div id="first">
		...
		<?php
			...
			$sql = "INSERT INTO fuwang (Date_to_be_booked, Full_Name, Contact_No, Gender, Address, Catagory, No_of_Rooms, Total_Room)
			...
			$sql ="update fuwang set Total_Room='10' where catagory='Delux Twin';";
			if ($conn->query($sql) === TRUE) {
	
			} else {
				echo "Error: " . $sql . "<br>" . $conn->error;
			}
			$sql ="update fuwang set Total_Room='15' where catagory='Delux';";
			if ($conn->query($sql) === TRUE) {
				
			} else {
				echo "Error: " . $sql . "<br>" . $conn->error;
			}
			$sql ="update fuwang set Total_Room='20' where catagory='Premium'";
			if ($conn->query($sql) === TRUE) {
